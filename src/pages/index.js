...
const Emoji = ({children}) => <span role="img" aria-label="emoji">{children}</span>
const IndexPage = () => (
  <StaticQuery
    query={graphql`
      {
    ...
    `}
    render={(data) => {
      const projects = data.allDataJson.edges.find(edge => edge.node.file === "projects").node.projects;
      ...
      and front-end development</p>
          <p>I'm a open source aficionado <Emoji>💻</Emoji>, powerlifting enthusiast <Emoji>🏋️‍♀</Emoji>, and coffee consumer <Emoji>☕</Emoji></p>
          <h2>Projects</h2>
          <ul>
            {projects.map((project, index) => <li key={index}>
              <a href={project.url}><strong>{project.name}</strong></a>
              <Emoji>{project.emoji}</Emoji>{` - ${project.description}`}
            </li>)}
          </ul>
